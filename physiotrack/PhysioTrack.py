...
'''
    ...
    Use PhysioTrack to compute your patient's pose, joint, and segment angles

    -----
    Help
    -----
    See https://github.com/yourusername/physiotrack
    Or run: physiotrack --help
    Or check: https://github.com/yourusername/physiotrack/blob/main/physiotrack/Demo/Config_demo.toml
    ...
    - Run on multiple videos with default parameters:
        physiotrack --video_input path_to_video1.mp4 path_to_video2.mp4
    ...
    - Run with custom parameters (all non specified are set to default): 
        physiotrack --show_plots False --time_range 0 2.1 --result_dir path_to_result_dir
        physiotrack --multiperson false --mode lightweight --det_frequency 50
    ...
    `conda create -n PhysioTrack python>=3.7`
    `conda activate PhysioTrack`
    pip install .
...
CONFIG_HELP =   {'config': ["C", "path to a toml configuration file"],
                ...
                'load_trc_px': ["", "load trc file to avoid running pose estimation again. false if not specified"],
                ...
                'device': ["", "Device for pose estimation can be 'auto', 'openvino', 'onnxruntime', 'opencv'"],
                ...
                'tracking_mode': ["", "'physiotrack' or 'deepsort'. 'deepsort' is slower, harder to parametrize but can be more robust if correctly tuned"],
...
__version__ = "0.1.0"
__maintainer__ = "PhysioTrack Team"
...
def base_params(config_dict):
    ...
    # video_dir and result_dir
    video_dir = config_dict.get('project', {}).get('video_dir', '')
    if video_dir == '' or video_dir is None: 
        video_dir = Path.cwd()
    else: 
...
def process(config='Config_demo.toml'):
    ...
    from physiotrack.process import process_fun
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Use PhysioTrack to compute your patient's pose, joint, and segment angles. See https://github.com/yourusername/physiotrack")
    ...
    PhysioTrack.process(new_config)
