...
def sort_people_physiotrack(keyptpre, keypt, scores=None):
    '''
    Associate persons across frames (PhysioTrack method)
    Persons' indices are sometimes swapped when changing frame
    A person is associated to another in the next frame when they are at a small distance
    
    N.B.: Requires min_with_single_indices and euclidian_distance function
...
def sort_people_deepsort(keypoints, scores, deepsort_tracker, frame, frame_count):
    ...
    try:
        ...
        desired_size = max(track_ids_frame) + 1 if track_ids_frame else 1
        sorted_keypoints = np.full((desired_size, keypoints.shape[1], 2), np.nan)
        sorted_scores = np.full((desired_size, scores.shape[1]), np.nan)
