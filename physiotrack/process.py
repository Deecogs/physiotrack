...
def setup_webcam(webcam_id, save_vid, vid_output_path, input_size):
    ...
    if save_vid:
        ...
        out_vid = cv2.VideoWriter(str(vid_output_path), fourcc, fps, (cam_width, cam_height))
...
def setup_video(video_file_path, save_vid, vid_output_path):
    ...
    try:
        ...
        if not cap.isOpened():
            raise ValueError(f"Could not open {video_file_path}")
    except:
...
def compute_floor_line(trc_data, keypoint_names = ['LBigToe', 'RBigToe'], toe_speed_below = 7):
    ...
    trc_data = trc_data[av_speeds>2.0]
...
def process_fun(config_dict, video_file, time_range, frame_rate, result_dir):
    ...
    px_to_m_from_person_id = int(config_dict.get('project', {}).get('px_to_m_from_person_id', 0))
    px_to_m_person_height_m = config_dict.get('project', {}).get('px_to_m_person_height', 1.7)
    ...
    if tracking_mode == 'deepsort':
        ...
        except: # if within single quotes instead of double quotes when run with physiotrack --mode """{dictionary}"""
    ...
    if video_file == "webcam":
        ...
        output_dir_name = f'{video_file_stem}_PhysioTrack'    
    ...
    if video_file == "webcam":
        ...
        logging.warning('Webcam input: the framerate may vary. If results are filtered, PhysioTrack will use the average framerate as input.')
    else:
    ...
    if load_trc_px:
        ...
        if tracking_mode not in ['deepsort', 'physiotrack']:
            logging.warning(f"Tracking mode {tracking_mode} not recognized. Using physiotrack method.")
    ...
    while cap.isOpened():
        ...
        for frame_nb in frame_iterator:
            ...
            if load_trc_px: 
                ...
                if tracking_mode == 'physiotrack': 
                    if 'prev_keypoints' not in locals(): prev_keypoints = keypoints
                    prev_keypoints, keypoints, scores = sort_people_physiotrack(prev_keypoints, keypoints, scores=scores)
        ...
        if to_meters:
            ...
            if calib_file:
                ...
                height_px = compute_height(trc_data[px_to_m_from_person_id].iloc[:,1:], keypoints_names,
                                            ...
                                            close_to_zero_speed=config_dict.get('kinematics').get('close_to_zero_speed_px'), 
